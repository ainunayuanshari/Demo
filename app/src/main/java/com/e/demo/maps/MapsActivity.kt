...
class MapsActivity : FragmentActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navigasi.setOnClickListener {
            ...
            intent.data = Uri.parse("https://www.google.com/maps/@$latitude,$latitude,17z")
            startActivity(Intent.createChooser(intent, "lanjutkan dengan.."))
        }
    }
    override fun onMapReady(p0: GoogleMap?) {
        val maps=p0
        val marker = maps?.addMarker(MarkerOptions().position(latLng!!).title(judulwisata.text.toString()))
